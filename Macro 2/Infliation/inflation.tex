...
\begin{frame}[allowframebreaks] % FIX
    ...
    \begin{center}
        \begin{tikzpicture}[scale=1.3, thick, dot/.style={circle,inner sep=1pt,fill,name=#1}]
            ...
            \draw [red] (2.5,3.5) -- (5,1) node [below right] {$IS(G_2)$};
            
            % LM curves
            \draw [blue] (0.5,1) -- (3.5,4) node [above left] {$LM(\frac{M}{P_1})$};
            \draw [red] (1.5,1) -- (4.5,4) node [above right] {$LM(\frac{M}{P_2})$};
          
            % Equilibrium points
            \node[dot=A] at (1.75,2.25) {};
            \node[dot=B] at (3.25,2.75) {};
          
            % Dotted lines
            \draw [dotted] (A) -- (1.75,0) node [below] {$Y_1$};
            \draw [dotted] (B) -- (3.25,0) node [below] {$Y_2$};
